...
from src.features.menu.setup import CMD_MODEL
from src.database.users import update_model, check_premium, get_model, delete_context
...
async def change_model_cb_(cb):
    ...
    if MODELS[model].premium_only and not (await check_premium(cb.from_user.id)):
        ...
        try:
            ...
            pass
        verdict = "👾 Модель изменена на " + model + verdict
